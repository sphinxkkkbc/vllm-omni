...
# ---------------------------------------------------------------------------
# 4. Bit-Exact when Input is padded
# ---------------------------------------------------------------------------


def _build_padded_decoder() -> OmniVoiceDecoder:
    from transformers import DacConfig, DacModel

    torch.manual_seed(42)

    decoder = OmniVoiceDecoder(OmniVoiceConfig())
    ...
    decoder.fc2 = nn.Linear(1024, 256).to(DEVICE).float()
    ...
    decoder.acoustic_decoder = DacModel(dac_config).decoder.to(DEVICE).float().eval()
...
def test_output_bit_exact_when_frames_are_padded():
    decoder = _build_padded_decoder()
    exact_inputs = torch.randint(
        0,
        1024,
        (1, 8, T_FRAMES),
        device=DEVICE,
    ...
    padded_length = [T_FRAMES]
    exact_output = decoder(exact_inputs)
    padded_output = decoder(padded_inputs, padded_length)
    padded_output = padded_output[:, :, : T_FRAMES * UPSAMPLE]
    torch.testing.assert_close(exact_output, padded_output)
