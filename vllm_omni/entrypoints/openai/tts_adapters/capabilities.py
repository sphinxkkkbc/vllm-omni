...
class TTSCapabilityLoader:
    """Load shared capability metadata without choosing model-specific policy."""

    def __init__(self, engine_client: Any) -> None:
        self.engine_client = engine_client

    def load_supported_speakers(self, config: Any = None) -> set[str]:
    ...
    def load_codec_frame_rate(self) -> float | None:
        ...
        try:
            rate = getattr(self.engine_client.model_config.hf_config, "codec_frame_rate_hz", None)
            if rate is not None:
    ...
    def load_precomputed_speakers(
        ...
        *,
        ...
        try:
            custom_voice_dir = getattr(self.engine_client.model_config.hf_config, "custom_voice_dir", None)
