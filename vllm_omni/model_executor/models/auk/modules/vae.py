...
from .vae_modules.bigvgan import activations
from .vae_modules.bigvgan.alias_free_torch import Activation1d
from .vae_modules.commons.layers import Conv1d, ConvTranspose1d
from .vae_modules.commons.ops import init_weights
from .vae_modules.vits.flows import ResidualCouplingBlock
...
class BigVGANFlowVAE(nn.Module):
    ...
    def inference_from_latents(self, x):
        if x.ndim != 3 or x.size(1) != self.h.latent_dim:
            raise ValueError(f"VAE decoder expects [B,D,T] with D={self.h.latent_dim}, got {tuple(x.shape)}")
...
# Backwards-compatible import name used by AuK stage plumbing.
AuKVAE = BigVGANFlowVAE

__all__ = ["AuKVAE", "BigVGANFlowVAE", "BigVGANFlowVAEConfig"]
