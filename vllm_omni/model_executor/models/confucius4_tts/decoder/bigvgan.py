...
class AMPBlock1(torch.nn.Module):
    """
    AMPBlock applies Snake / SnakeBeta activation functions with trainable
    parameters that control periodicity, defined for each layer.
    AMPBlock1 has additional self.convs2 that contains additional Conv1d
    layers with a fixed dilation=1 followed by each layer in self.convs1

    Args:
        h (AttrDict): Hyperparameters.
        channels (int): Number of convolution channels.
        kernel_size (int): Size of the convolution kernel. Default is 3.
        dilation (tuple): Dilation rates for the convolutions. Each dilation
            layer has two convolutions. Default is (1, 3, 5).
...
class BigVGAN(
    ...
    Args:
        h (AttrDict): Hyperparameters.
        use_cuda_kernel (bool): If set to True, loads optimized CUDA kernels for
        AMP. This should be used for inference only, as training is not
        supported with CUDA kernels.

    Note:
        - The `use_cuda_kernel` parameter should be used for inference only, as
          training with CUDA kernels is not supported.
    ...
    def _from_pretrained(
        ...
        **model_kwargs,
        ...
        if use_cuda_kernel:
            print(
                "[WARNING] You have specified use_cuda_kernel=True during "
                "BigVGAN.from_pretrained(). Only inference is supported "
                "(training is not implemented)!"
            )
            print(
                "[WARNING] You need nvcc and ninja installed in your system "
                "that matches your PyTorch build is using to build the kernel. "
                "If not, the model will fail to initialize or generate "
                "incorrect waveform!"
            )
            print(
                "[WARNING] For detail, see the official GitHub repository: "
                "https://github.com/NVIDIA/BigVGAN?tab=readme-ov-file"
